...
export function MiniSpark({ data, color = 'var(--pos-soft)', width = 72, height = 26 }: MiniSparkProps) {
  ...
  // Center a single point; otherwise spread evenly. Guard the value range so a
  // flat series (max === min) doesn't divide by zero.
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} style={{ display: 'block', overflow: 'visible' }}>
      ...
      <circle cx={x(n - 1)} cy={y(data[n - 1])} r="2.2" style={{ fill: color }} />
    </svg>
  );
}
