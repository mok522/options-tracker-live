/* Sample data — modeled on a ThinkOrSwim account-statement CSV export.
   Hand-tuned (not random) so every chart looks intentional. Exposed on window. */

window.OTT = (function () {
  // ── headline KPIs (YTD 2026) ───────────────────────────────
  const kpis = {
    netPL:        24847.50,
    netPLpct:     18.6,        // vs starting equity
    winRate:      67.6,
    winRateDelta: 3.2,
    closedTrades: 142,
    openTrades:   11,
  ...
  // ── monthly realized P&L (trailing 12 mo, Jul'25 → Jun'26) ──
  const monthly = [
  ...
  // ── strategy breakdown (share of realized P&L) ─────────────
  const strategies = [
  ...
  // ── cumulative P&L curve (≈ weekly marks across the year) ──
  // monotonic-ish equity curve ending at netPL
  const cumulative = [
  ...
  // ── tax exposure (feeds the tax tab; light use on dashboards) ─
  const tax = {
    shortTerm: 16240,
    longTerm:   3120,
    s1256: 5480,          // 60/40 SPX/NDX/RUT
    washFlags: 2,
    estTax: 4910,
  };
  ...
  return { kpis, monthly, strategies, symbols, cumulative, trades, tax, fmtUSD, fmtSigned, fmtNum };
})();
